...
from tests.current_image_contract_fixture import current_art_direction, current_image_direction, current_image_task, current_product_feature, supported_review_results
...
def check_child_components(case):
    source = {'source_id': 'source_06', 'source_index': 6, 'role': 'scene', 'source_sha256': 'a'*64,
              ...
              'observation': {'product_features': [current_product_feature()], 'product_extent': 'whole_view', 'objects': [], 'text_gaps': [],
                              'reference_purposes': ['appearance'], 'evidence_gaps': []}}
    ...
    with tempfile.TemporaryDirectory() as tmp, patch('core.visual_design_kit.review_planning_bindings', side_effect=supported_review_results) as reviewer, patch(
        ...
        planned = _finish_image_briefs(raw, output_inventory=initial_output_inventory([source]), job=Path(tmp), child='B1', source_manifest=[source], category_id='bed_frame',
    ...
    invalid = compile_visual_design_kit_response(unknown, output_inventory=initial_output_inventory([source]), source_manifest=[source])
...
def _check_shared_leaf_scope(case, source, raw):
    ...
    source['observation']['product_features'][0]['physical_facts'] = ['The sold frame has a white painted finish']
    raw['family_art_direction']['palette_direction']['room']['wall'] += '; repaint the sold frame to match this wall'
        ...
        for request in requests:
            ...
            if request['role_design']['role'] == 'scene' and shared.get(leaf) != changed:
                result[request['key']]['findings'].append(dict(operation='shared_design:' + leaf,
                    status='contradiction', reason='The wall instruction also repaints the sold frame',
                    fact_ids=['physical:' + source['source_id'] + ':frame_support:0']))
        return result
    for edits, accepted in (({leaf: changed}, True),
        ...
        with tempfile.TemporaryDirectory() as tmp, patch('core.visual_design_kit.review_planning_bindings', side_effect=review), patch(
            ...
            fixed = _finish_image_briefs(raw, output_inventory=initial_output_inventory([source]), job=Path(tmp), child='B1', source_manifest=[source], category_id='bed_frame',
                source_paths=[], source_originals=[], trace_dir=Path(tmp), deadline_monotonic=time.monotonic()+5, cached=None)
...
def _check_execution_conflicts(case, source, raw):
    ...
    for wrong in ('Show a black metal bed frame', 'Make the frame 60 inches wide', 'Use blue bedding instead of the assigned component color'):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch('core.visual_design_kit.review_planning_bindings', side_effect=review), patch(
            ...
            result = _finish_image_briefs(planned, output_inventory=initial_output_inventory([source]), job=Path(tmp), child='B1', source_manifest=[source], product_claims=claims,
        ...
        for request in requests:
            if 'product' in request['shared_design']['palette_direction']:
                results[request['key']].update(status='contradiction', findings=[{
                    ...
                    'reason': 'Sold frame finish is not editable staging', 'fact_ids': [claims[0]['evidence_id']]}])
        return results
...
def _check_p4_contracts(case):
    ...
    source['observation']['product_extent'] = 'detail'
    ...
    with case.assertRaisesRegex(ValueError, 'Whole-product output needs'):
    ...
    direction['product_sources'].append('source_07')
    validate_image_brief_draft(draft, source, art, [source, whole], 'bed_frame')
    earlier = _evidence_source('size', source_id='source_01')
    case.assertEqual('source_07', task_specs({}, [earlier, whole], inventory=initial_output_inventory([earlier, whole]))[0]['source']['source_id'])
    case.assertEqual('source_01', task_specs({}, [earlier], inventory=initial_output_inventory([earlier]))[0]['source']['source_id'])
    ...
    size['image_direction']['measurement_ids'] = ['source_00:width']
